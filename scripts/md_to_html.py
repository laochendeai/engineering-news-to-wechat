...
def basic_md_to_html(md_text: str) -> str:
    ...
    while i < len(lines):
        ...
        # Unordered list (supports indented continuation lines)
        ...
        if m:
            ...
            while j < len(lines):
                ...
                if not nxt_stripped:
                    break
                if re.match(r'^\s{2,}\S+', nxt) and not re.match(r'^\s{2,}(?:[-*+]|\d+\.)\s+', nxt):
                    item_parts.append(nxt_stripped)
                    j += 1
                    continue
                break
            html_lines.append(f'<li style="{STYLES["li"]}">{inline(" ".join(item_parts))}</li>')
            i = j
            continue

        # Ordered list (supports indented continuation lines)
...
def normalize_list_continuations(md_text: str) -> str:
    """Fold indented continuation lines into the preceding list item.

    This avoids renderer differences where ordered/unordered list items render with
    a marker but drop following indented body lines in WeChat-oriented pipelines.
    """
    lines = md_text.split("\n")
    out = []
    i = 0
        ...
        if re.match(r'^\d+\.\s+.+', stripped) or re.match(r'^[-*+]\s+.+', stripped):
            ...
            while j < len(lines):
                ...
                if re.match(r'^\s{2,}\S+', nxt) and not re.match(r'^\s{2,}(?:\d+\.|[-*+])\s+', nxt):
                    ...
                    j += 1
                    continue
                # a new list item of different list type: force a blank line so markdown won't merge them
                if (is_ordered and re.match(r'^[-*+]\s+.+', nxt_stripped)) or ((not is_ordered) and re.match(r'^\d+\.\s+.+', nxt_stripped)):
                    break
                break
            out.append(' '.join(parts))
            # separate mixed list types to avoid markdown "extra" merging them into one <ol>
            if j < len(lines):
...
def convert(md_path: str, image_map_path: str = None) -> str:
    ...
    md_text = normalize_list_continuations(md_text)
